...
class CustomerReportController extends BackendController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($_POST) {
            ...
            if($user_id && !blank($dateBetween)) {
                // $role                = Role::find(2);
                $this->data['userdetails'] = User::where('id', $user_id)->whereBetween('created_at', [$dateBetween['from_date'], $dateBetween['to_date']])->latest()->get();
            } elseif($user_id) {
                // $role                = Role::find(2);
                $this->data['userdetails'] = User::where('id', $user_id)->latest()->get();
            } elseif( !blank($dateBetween)){
                // $role                = Role::find(2);
                $this->data['userdetails'] = User::whereBetween('created_at', [$dateBetween['from_date'], $dateBetween['to_date']])->latest()->get();
            }else{
                // $role                = Role::find(2);
                $this->data['userdetails'] = User::latest()->get();
            }

        }
        return view('restaurant.report.customerReport.index', $this->data);
    }
    ...
    public function pdf($set_user_id, $set_from_date = '', $set_to_date = '')
    {
        ...
        $pdf = PDF::loadView('restaurant.report.customerReport.pdf', $this->data);
