...
class OrderController extends BackendController
{
    ...
    public function index()
    {
        ...
        return view('restaurant.orders.index', $this->data);
    }
    ...
    public function show($id)
    {
        ...
        return view('restaurant.orders.view', $this->data);
    }
    ...
    public function delivery($id)
    {
        ...
        return view('restaurant.orders.delivery', $this->data);
    }
    ...
    public function edit($id)
    {
        ...
        return view('restaurant.orders.edit', $this->data);
    }
    ...
    public function update(OrderRequest $request, $id)
    {
        ...
        if ($orderService->status) {
            ...
            return redirect(route('restaurant.orders.index'))->withSuccess('Order successfully updated');
        } else {
            return redirect(route('restaurant.orders.index'))->withError($orderService->message);
        }
    }
    ...
    public function productReceive(Request $request, $id)
    {
        ...
        if ($productReceive->status) {
            return redirect(route('restaurant.orders.show', $id))->withSuccess('The Data Updated Successfully');
        } else {
            return redirect(route('restaurant.orders.show', $id))->withError($productReceive->message);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect(route('restaurant.orders.index'))->withSuccess('The Data Deleted Successfully');
    }
                ...
                ->addColumn('action', function ($order) {
                    ...
                    // if (auth()->user()->can('orders_show')) {
                        ...
                        $retAction .= '<a href="' . route(
                            'orders.show',
                            $order
                        ) . '" class="btn btn-sm btn-icon btn-info" data-toggle="tooltip" data-placement="top" title="View"><i class="lar la-eye"></i></a>';
                    }

                    // if ($order->statusData && auth()->user()->can('orders_edit') && $order->status != OrderStatus::COMPLETED) {
                        if (true) {
                        $retAction .= '&nbsp;&nbsp;&nbsp;<a href="' . route('orders.edit', $order) . '" class="pl-2 btn btn-sm btn-icon btn-primary" data-toggle="tooltip" data-placement="top" title="Edit"><i class="lar la-edit"></i></a>';
                    }

                    // if (auth()->user()->can('orders_show') && $order->delivery_boy_id && auth()->user()->id != $order->delivery_boy_id) {
                    if (true) {
                        $retAction .= '&nbsp;&nbsp;&nbsp;<a href="' . route('orders.delivery', $order) . '" class="btn btn-sm btn-icon btn-success" data-toggle="tooltip" data-placement="top" title="Delivery"><i class="lar la-list-alt"></i></a>';
                    }
    ...
    {
        return view('restaurant.orders.liveOrders', $this->data);
    }

    public function getLiveOrders()
    {
        $orderCount            = Order::whereDate('created_at', Carbon::today())->orderowner()->orderBy('id', 'desc')->get();
        ...
        $orders = Order::whereDate('created_at', Carbon::today())->orderowner()->orderBy('id', 'desc')->get();
        foreach ($orders as $order) {
            if ($order->status == OrderStatus::PENDING) {
        ...
        return view('restaurant.orders.live_order_data', $this->data);
    }
}
