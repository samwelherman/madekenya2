...
use App\Models\rastaurant\Reservation;
use Illuminate\Http\Request;
use App\Enums\RestaurantStatus;
use App\Enums\DeliveryHistoryStatus;
use App\Models\restaurant\DeliveryStatusHistories;
...
class DashboardController extends BackendController
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->myrole == 3 && auth()->user()->restaurant) {
            $ownerTotalOrders = Order::orderBy('id', 'desc')->where('restaurant_id', auth()->user()->restaurant->id)->get();
            $ownerTotalReservations = Reservation::orderBy('id', 'desc')->where('restaurant_id', auth()->user()->restaurant->id)->get();
            $ownerNotificationOrders = Order::orderBy('id', 'desc')->where('restaurant_id', auth()->user()->restaurant->id)->where('status',OrderStatus::PENDING)->get();
        }
        ...
        $notificationOrders = Order::where(['delivery_boy_id' => null])->whereIn('status', [OrderStatus::ACCEPT, OrderStatus::PROCESS])->latest()->whereNotIn('id', $deliveryStatusHistoriesArray)->get();


        $totalDaliveryOrders = Order::where(['delivery_boy_id' => auth()->user()->id])->latest()->get();
        // $role  = Role::find(2);
        // $totalUsers = User::role($role->name)->where(['status' => UserStatus::ACTIVE])->latest()->get();
        $totalUsers = User::where(['status' => UserStatus::ACTIVE])->latest()->get();
        ...
        $this->data['userCredit']            =  0 ;
        // $this->data['userCredit']            = auth()->user()->balance->balance > 0 ? auth()->user()->balance->balance : 0 ;

        return view('restaurant.dashboard.index', $this->data);
    }
